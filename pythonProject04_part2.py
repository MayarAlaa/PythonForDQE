...
def append_word(capitalized_result):
...
##################################################################
#It iz misspelling here. Fix“iz” with correct “is”, but only when it iz a mistake.
fix_misspell = lambda capitalized_result, pattern , replacement : re.sub(pattern, replacement, capitalized_result)
...
def count_whitespaces(fixed_result):
  pattern = r'[ \t\r\f\v\n]'
...
capitalized_result = append_word(capitalize_sentences(text))

fixed_text =  fix_misspell(capitalized_result , r' iz ' , ' is ')

count_whitespaces(fixed_text)